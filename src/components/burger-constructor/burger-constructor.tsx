...
import { AddIngredientToBurgerAction, ChangeBunAction, ChangeIngredientsOrderAction, RemoveIngredientFromBurgerAction } from '../../services/actions/constructor-ingredients';
...
function BurgerConstructor() {
    ...
    const { order } = useSelector(store => store.apiOrder);
    ...
    return (<>
        <div ref={dropTarget}>
            ...
            <div className={`${styles.totalContainer} ${(!bun && styles.hiddenContainer)}`}>
                ...
                <Button htmlType='button' type='primary' size='medium' extraClass='ml-10' onClick={handleCreateOrder}>Оформить заказ</Button>
            </div>
        </div>
        <div style={{ overflow: 'hidden' }}>
            {modalVisible && !!order && <OrderDetails onClose={handleCloseModal} orderNum={order.order?.number} />}
        </div>
    </>
    );
}
